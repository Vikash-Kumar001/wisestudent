...
import {
  getReplayCostForGame,
  getGameIndexFromId,
} from "../../../utils/parentGameUtils";
...
const ParentGameCategoryPage = () => {
  ...
  const handleGameClick = async (game) => {
    ...
    if (
      ...
      game.id !== "parent-education-70" &&
      game.id !== "parent-education-80" &&
      game.id !== "parent-education-90" &&
      game.id !== "parent-education-100" &&
      game.gameIndex > 1
    ) {
      const previousGameId = `parent-education-${game.gameIndex - 1}`;
      const previousGameCompleted =
        gameCompletionStatus[previousGameId] === true;

      if (!previousGameCompleted) {
        const previousGame = games.find((g) => g.id === previousGameId);
        toast.error(
          `Complete "${
            previousGame?.title || "the previous game"
          }" first to unlock this game!`
        );
        return;
      }
    }
    ...
    if (game.id === "parent-education-10") {
      ...
      const allCompleted = requiredGameIds.every(
        (id) => gameCompletionStatus[id] === true
      );

      if (!allCompleted) {
        toast.error(
          "Complete all 9 awareness games first to unlock this badge!"
        );
        return;
      }
    }
    ...
    if (game.id === "parent-education-20") {
      ...
      if (!allCompleted) {
        toast.error(
          "Master all stress regulation practices first to unlock this badge!"
        );
        return;
      }
    }
    ...
    if (game.id === "parent-education-40") {
      ...
      if (!allCompleted) {
        toast.error(
          "Complete all 5 presence and balance activities first to unlock this badge!"
        );
        return;
      }
    }
    ...
    if (game.id === "parent-education-50") {
      ...
      if (!allCompleted) {
        toast.error(
          "Complete all 5 mindfulness activities first to unlock this badge!"
        );
        return;
      }
    }
    ...
    if (game.id === "parent-education-60") {
      ...
      if (!allCompleted) {
        toast.error(
          "Complete all 5 resilience activities first to unlock this badge!"
        );
        return;
      }
    }
    ...
    if (game.id === "parent-education-70") {
      ...
      if (!allCompleted) {
        toast.error(
          "Complete all 5 communication activities first to unlock this badge!"
        );
        return;
      }
    }
    ...
    if (game.id === "parent-education-80") {
      ...
      if (!allCompleted) {
        toast.error(
          "Complete all 5 connection activities first to unlock this badge!"
        );
        return;
      }
    }
    ...
    if (game.id === "parent-education-90") {
      ...
      if (!allCompleted) {
        toast.error(
          "Complete all 5 purpose activities first to unlock this badge!"
        );
        return;
      }
    }
    ...
    if (game.id === "parent-education-100") {
      ...
      if (!allCompleted) {
        toast.error(
          "Complete all 5 self-care activities first to unlock this badge!"
        );
        return;
      }
    }
  ...
  const handleUnlockReplay = async () => {
    ...
    try {
      const result = await parentGameCompletionService.unlockReplay(
        game.id,
        gameIndex
      );
      ...
      const errorMessage =
        ...
        "Failed to unlock replay. Please try again.";
  ...
  const getGameStatus = (game) => {
    ...
    if (game.id === "parent-education-10") {
      ...
      const allRequiredCompleted = requiredGameIds.every(
        (id) => gameCompletionStatus[id] === true
      );
    ...
    const needsReplayUnlock =
      isCompleted && (!progress || progress.replayUnlocked !== true);
    ...
    if (game.gameIndex > 1) {
      const previousGameId = `parent-education-${game.gameIndex - 1}`;
      const previousGameCompleted =
        gameCompletionStatus[previousGameId] === true; // Only true if previous game fully completed
              ...
              return (
                <motion.div
                  ...
                  className={`group relative overflow-hidden rounded-3xl border-2 ${
                    isActive ? "border-gray-800" : cardGradient.border
                  } bg-gradient-to-br ${
                    cardGradient.bg
                  } shadow-xl cursor-pointer transition-all hover:shadow-2xl ${
                    ...
                    <div
                      ...
                      style={{
                        background: `linear-gradient(45deg, ${
                          gradients[index % gradients.length].icon.includes(
                            "purple"
                          )
                            ...
                            : "rgba(236, 72, 153, 0.3), rgba(249, 115, 22, 0.2)"
                        }, transparent)`,
                      }}
                    />
                  )}
                        ...
                        >
                          ...
                          <span>
                            {game.id === "parent-education-10"
                              ...
                              : game.id === "parent-education-90"
                              ? "Complete All Purpose Activities to Unlock"
                              : "Master Self-Care to Unlock"}
                          </span>
                        </motion.button>
                      ) : (game.isBadgeGame ||
                          game.id === "parent-education-10" ||
          ...
          >
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-4 space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-purple-600 font-bold">
                  {getReplayCostForGame(
                    getGameIndexFromId(selectedGameForReplay.id)
                  )}{" "}
                  Healcoins
                </span>
              </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-6">
              <p className="text-sm text-amber-800 leading-relaxed">
                <strong>Note:</strong> You won't earn coins when replaying this
                game. The game will be locked again after you complete the
                replay.
              </p>
            </div>
