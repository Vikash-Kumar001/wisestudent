...
const PuzzleEmotionMatch = () => {
  ...
  const correctMatches = [
    { leftId: 1, rightId: 1 }, // Fear → Heart Racing (left 1st, right 2nd)
    { leftId: 2, rightId: 2 }, // Calm → Deep Breathing (left 2nd, right 1st)
    { leftId: 3, rightId: 3 }, // Anger → Clenched Fists (left 3rd, right 4th)
    { leftId: 4, rightId: 4 }, // Sadness → Tears (left 4th, right 3rd)
    { leftId: 5, rightId: 5 }  // Surprise → Wide Eyes (left 5th, right 5th)
  ];
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Puzzle: Emotion Match game completed! Score: ${score}/${leftItems.length}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
  ...
  return (
    <GameShell
      ...
      score={score}
      subtitle={showResult ? "Game Complete!" : `Match emotions with their physical responses (${matches.length}/${leftItems.length} matched)`}
        ...
        {!showResult ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/20">
              <h3 className="text-lg md:text-xl font-bold text-white mb-3 md:mb-4 text-center">Emotions</h3>
            ...
            {/* Right column - Physical Responses */}
            <div className="bg-white/10 backdrop-blur-md rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/20">
              <h3 className="text-lg md:text-xl font-bold text-white mb-3 md:mb-4 text-center">Physical Responses</h3>
              <div className="space-y-3 md:space-y-4">
                {rightItems.map(item => {
            ...
            {score >= 3 ? (
              <div>
                ...
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You correctly matched {score} out of {leftItems.length} emotions!
                  You understand how emotions connect to physical responses!
                </p>
              ...
              <div>
                ...
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You matched {score} out of {leftItems.length} emotions correctly.
                  Remember, different emotions trigger different physical responses!
                </p>
              </div>
            )}
          </div>
