...
const PuzzlePositivePractices = () => {
  const location = useLocation();
  
  // Get game data from game category folder (source of truth)
  const gameId = "brain-teens-54";
  ...
  const correctMatches = [
    { leftId: 1, rightId: 1 }, // Meditation → Calm (left 1st, right 2nd)
    { leftId: 2, rightId: 2 }, // Affirmation → Confidence (left 2nd, right 1st)
    { leftId: 3, rightId: 3 }, // Gratitude Journal → Appreciation (left 3rd, right 4th)
    { leftId: 4, rightId: 4 }, // Exercise → Energy (left 4th, right 3rd)
    { leftId: 5, rightId: 5 }  // Deep Breathing → Relaxation (left 5th, right 5th)
  ];
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Puzzle: Positive Practices game completed! Score: ${score}/${leftItems.length}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
  ...
  return (
    <GameShell
      ...
      score={score}
      subtitle={showResult ? "Game Complete!" : `Match positive practices with their benefits (${matches.length}/${leftItems.length} matched)`}
      ...
      <div className="space-y-6 md:space-y-8 max-w-5xl mx-auto px-4">
        {!showResult ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            {/* Left column - Positive Practices */}
            <div className="bg-white/10 backdrop-blur-md rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/20">
              <h3 className="text-lg md:text-xl font-bold text-white mb-3 md:mb-4 text-center">Positive Practices</h3>
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/20">
              <h3 className="text-lg md:text-xl font-bold text-white mb-3 md:mb-4 text-center">Benefits</h3>
              <div className="space-y-3 md:space-y-4">
                {rightItems.map(item => {
            ...
            {score >= 3 ? (
              <div>
                ...
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You correctly matched {score} out of {leftItems.length} practices!
                  You understand how positive practices lead to positive outcomes!
                </p>
              ...
              <div>
                ...
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You matched {score} out of {leftItems.length} practices correctly.
                  Remember, positive practices lead to positive outcomes!
                </p>
              </div>
            )}
          </div>
...
export default PuzzlePositivePractices;
