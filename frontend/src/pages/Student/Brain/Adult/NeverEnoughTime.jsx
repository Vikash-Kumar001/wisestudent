...
const NeverEnoughTime = () => {
  const location = useLocation();
  const gameId = "brain-adults-7"; // Seventh brain-adults game
  ...
  useEffect(() => {
    if (skillUnlocked) {
      console.log("Skill unlocked: Time pressure awareness");
    }
  }, [skillUnlocked]);

  useEffect(() => {
    if (skillUnlocked) {
      console.log("Completed Never Enough Time", { score });
    }
  }, [skillUnlocked, score]);
    ...
    >
      <div className="space-y-6 max-w-4xl mx-auto p-4">
        ...
        <Section title="Stress Point">
          Constant rushing and unfinished tasks.
        </Section>
        ...
        {skillUnlocked && (
          <Section title="Skill / Insight Unlocked">
            <strong>Time pressure awareness</strong>
          </Section>
        )}
...
export default NeverEnoughTime;