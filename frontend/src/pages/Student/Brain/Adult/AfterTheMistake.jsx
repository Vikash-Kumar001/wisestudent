...
const AfterTheMistake = () => {
  const location = useLocation();
  const gameId = "brain-adults-18"; // Eighteenth brain-adults game
  ...
  useEffect(() => {
    if (skillUnlocked) {
      console.log("Skill unlocked: Mistake response awareness");
    }
  }, [skillUnlocked]);

  useEffect(() => {
    if (skillUnlocked) {
      console.log("Completed After the Mistake", { score });
    }
  }, [skillUnlocked, score]);
      ...
      <div className="space-y-6 max-w-4xl mx-auto p-4">
        ...
        {skillUnlocked && (
          <Section title="Skill / Insight Unlocked">
            <strong>Mistake response awareness</strong>
          </Section>
        )}
...
export default AfterTheMistake;