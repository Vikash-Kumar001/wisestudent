...
const totalStages = FAKE_PAYMENT_SCREENSHOT_STAGES.length;
...
const FakePaymentScreenshot = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = FAKE_PAYMENT_SCREENSHOT_STAGES[currentStage];
  ...
  const stage = FAKE_PAYMENT_SCREENSHOT_STAGES[Math.min(currentStage, totalStages - 1)];
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: FAKE_PAYMENT_SCREENSHOT_STAGES[currentStage].id, isCorrect: FAKE_PAYMENT_SCREENSHOT_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
