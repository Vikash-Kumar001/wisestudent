...
const totalStages = EARNING_READINESS_STAGES.length;
...
const EarningReadinessCheckpoint = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = EARNING_READINESS_STAGES[currentStage];
  ...
  const stage = EARNING_READINESS_STAGES[Math.min(currentStage, totalStages - 1)];
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: EARNING_READINESS_STAGES[currentStage].id, isCorrect: EARNING_READINESS_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
