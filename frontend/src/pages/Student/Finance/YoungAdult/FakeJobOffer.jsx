...
const totalStages = FAKE_JOB_OFFER_STAGES.length;
...
const FakeJobOffer = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = FAKE_JOB_OFFER_STAGES[currentStage];
  ...
  const stage = FAKE_JOB_OFFER_STAGES[Math.min(currentStage, totalStages - 1)];
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: FAKE_JOB_OFFER_STAGES[currentStage].id, isCorrect: FAKE_JOB_OFFER_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
