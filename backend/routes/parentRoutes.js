...
const buildParentAnalyticsPdf = (analytics, options = {}) =>
  new Promise((resolve, reject) => {
    try {
      ...
      const addPageHeader = () => {
        ...
        doc.fillColor(colors.subText).font("Helvetica-Bold").fontSize(9).text("Child Analytics Report", marginX, 10);
      };
      ...
      const drawHeader = () => {
        const childName = analytics?.childCard?.name || options.childName || "Child";
        drawGradientBand(0, 110, colors.primary, colors.accent);
        doc.rect(0, 110, pageWidth, 8).fill(colors.deep);
        doc.fillColor("white").font("Helvetica-Bold").fontSize(20).text(`${childName} Analytics`, marginX, 30);
        const subtitle = `Parent Report | ${new Date().toLocaleDateString()}`;
      ...
      const drawDonutChart = (entries, size = 200) => {
        const chartHeight = size + 24;
        ensureSpace(chartHeight);
        const centerX = marginX + size / 2;
        ...
        if (total > 0) {
          entries.forEach((entry) => {
            ...
            doc
              ...
              .arc(centerX, centerY, radius, toRad(startAngle), toRad(endAngle))
              .lineTo(centerX, centerY)
              .fill();
            startAngle = endAngle;
          });
        } else {
          doc.circle(centerX, centerY, radius).fill(colors.muted);
        }

        doc.circle(centerX, centerY, innerRadius).fill(colors.light);

        const legendX = marginX + size + 24;
        let legendY = y + 4;
        const legendMaxWidth = pageWidth - legendX - marginX;
        entries.forEach((entry) => {
          ensureSpace(14);
          doc.rect(legendX, legendY + 2, 10, 10).fill(entry.color);
          doc.fillColor(colors.text).font("Helvetica").fontSize(9).text(
            `${entry.label} (${entry.value}%)`,
            legendX + 16,
            legendY,
            { width: legendMaxWidth, ellipsis: true }
          );
          legendY += 14;
        });
      ...
      const snapshot = analytics?.snapshotKPIs || {};
      const summaryCards = [
        { label: "Level", value: analytics?.level ?? 1 },
        { label: "Total XP", value: analytics?.xp ?? 0 },
        { label: "Streak", value: `${analytics?.streak ?? 0} days` },
        {
          label: "HealCoins",
          value: typeof analytics?.healCoins === "object"
            ? analytics?.healCoins?.currentBalance ?? analytics?.healCoins?.balance ?? 0
            : analytics?.healCoins ?? 0,
        },
        { label: "Games Done", value: snapshot?.totalGamesCompleted ?? 0 },
        { label: "This Week", value: `${snapshot?.totalTimeSpent ?? 0}m` },
      ];

      addSectionHeader("Snapshot Summary");
      drawSummaryCards(summaryCards);

      const skillsDistribution = analytics?.skillsDistribution || {};
      ...
      const filteredPillars = pillarOrder.filter((pillar) => {
      ...
      addSectionHeader("Child Growth Mastery");
      filteredPillars.forEach((pillar) => {
        drawPillarRow(pillar, Number(byPillar[pillar]) || 0);
      });

      addSectionHeader("Skills Distribution");
      drawDonutChart(pillarEntries, 180);

      addSectionHeader("Weekly Engagement Summary");
      const weekly = analytics?.weeklyEngagement || {};
      const engagementCards = [
        { label: "Games Played", value: weekly?.gameSessions ?? 0 },
        { label: "Total Time", value: `${weekly?.totalMinutes ?? 0}m` },
      ];
      drawSummaryCards(engagementCards);

      const achievements = analytics?.recentAchievements || [];
      addSectionHeader("Recent Achievements");
      if (achievements.length === 0) {
        ensureSpace(18);
        doc.fillColor(colors.subText).font("Helvetica").fontSize(9).text("No achievements available.", marginX, y);
        y += 16;
      } else {
        const rows = achievements.slice(0, 8).map((achievement) => [
          achievement.game || "Game",
          achievement.category || "Category",
          achievement.achievement || "Achievement",
        ]);
        drawTable(["Game", "Category", "Achievement"], rows, [170, 120, 160]);
      }

      const footerY = pageHeight - marginBottom - 12;
      doc.fillColor(colors.subText).font("Helvetica").fontSize(8).text("Confidential - WiseStudent Parent Analytics", marginX, footerY);
      doc.text(`Generated ${new Date().toLocaleString()}`, marginX, footerY, { align: "right" });
...
router.post("/child/:childId/report", async (req, res) => {
  try {
    const { childId } = req.params;
    const { format = "pdf", type = "progress", analytics } = req.body;
    const parentId = req.user._id;
    ...
    if (format === "pdf") {
      const pdfBuffer = await buildParentAnalyticsPdf(analytics, {
        childName: child.name,
      });
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=analytics-report-${childId}-${fileDate}.pdf`
      );
